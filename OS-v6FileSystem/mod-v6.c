...
//set current directory at initfs
//
...
int file_descriptor;
/*
 * total free data blocks available
 * so we can check if there's enough memory before we start writing a file
 * update in cpin, rm and mkdir
 */
...
void initfs(int total_blocks, int total_inode_blocks) {
	...
	free_data_blocks = total_blocks - (total_inode_blocks + 2);
...
/*    for (int i = 0; i < num_of_blocks; i++) {
    ...
    int inode_offset = getFreeInode();
    //set fields except address
    inode.flags = (INODE_ALLOC | BLOCK_FILE); 		// I-node allocated + directory file
    inode.nlinks = 1;
    inode.uid = 0;
    inode.gid = 0;
    inode.size0 = (int)((file_size & 0xFFFFFFFF00000000) >> 8);
    ...
    write(file_descriptor, &inode, 64);
